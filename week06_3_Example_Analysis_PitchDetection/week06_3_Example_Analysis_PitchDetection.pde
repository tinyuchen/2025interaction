...
// 想要把聲音的最重要頻率 (Hz) 找出來，使用 PitchDetection 物件
/**
...
SoundFile sound;  // 新加入這行，因為要把音樂 beat.mp3 讀進來
...
void setup() {
  ...
  sound = new SoundFile(this, "beat.mp3");  // 新加入這行，因為要把音樂 beat.mp3 讀進來
  sound.loop();
  pitchDetector = new PitchDetector(this);  // 新增、初始化 PitchDetection 物件
  // capture audio input
  // pitchDetector.input(new AudioIn(this, 0));  // 原本程式是用 Audio In 音效卡的音源輸入
...
void draw() {
  ...
  if (pitchAndConfidence[1] > 0) {
    ...
    println(pitchAndConfidence[0], pitchAndConfidence[1]); // 把 pitch 和 confidence 都印出來
  }
